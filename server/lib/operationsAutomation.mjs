...
export async function createOperationsPlan(context = {}, scope = "all") {
  ...
  let warning = "The planning summary is unavailable; verified-data scheduling and checks remain active.";
  ...
  try {
    ...
    console.warn("Operations narrative failed:", error.message);
    warning = "The planning summary could not be prepared. Verified-data scheduling and checks remain active.";
  }
...
function buildProduction({ items, selectedQuote, controlQuote, supplier, leadDays, startDate, deliveryDate, updates, inspections }) {
  ...
  return {
    ...
    forecastDisclaimer:
      "These dates are planning forecasts only. The supplier must submit the real factory schedule and Cho must approve it before any date becomes an active production baseline.",
  ...
  return {
    packing: {
      ...
      nextAction: missingPackingInputs.length
        ? "Collect packed dimensions and weight, then run Loading Planner."
        : "Run Loading Planner and submit the computed plan to Cho for approval."
    },
    documentControl: {
    ...
    handover: {
      ...
      checklist: [
        ...
        "Prepare the archive and obtain the client’s signature; the client must sign personally."
      ]
    },
    automationRules: [
...
function shipmentAlert(shipment, deliveryDate) {
  if (!shipment) return "No live carrier booking has been recorded; shipment tracking is not yet available.";
