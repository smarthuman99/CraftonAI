...
function App() {
  const [currentView, setCurrentStageView] = useState("Home"); // Views: "Home", "CaseStudy", "OurStory", "Contact", "Auth", "Intake", "ClientPortal", "Backoffice"
  const [user, setUser] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem("crafton_user") || "null");
    } catch (e) {
      return null;
    }
  });
  const [authTab, setAuthTab] = useState("register"); // "login" or "register"
  const [authRedirectView, setAuthRedirectView] = useState("Home");
  
  // Auth form fields
  const [authEmail, setAuthEmail] = useState("");
  const [authPassword, setAuthPassword] = useState("");
  const [authName, setAuthName] = useState("");
  const [authChannel, setAuthChannel] = useState("WhatsApp");
  const [authContactId, setAuthContactId] = useState("");
  const [authError, setAuthError] = useState("");

  // Intake (Start a project) states
  const [intakeTab, setIntakeTab] = useState("pdf"); // "pdf", "excel", "text", "form"
  const [uploadedFile, setUploadedFile] = useState(null);
  const [uploadProgress, setUploadProgress] = useState(0);
  const [isUploading, setIsUploading] = useState(false);
  const [intakeNotes, setIntakeNotes] = useState("");

  // Manual Form spec fields
  const [manualTypeCn, setManualTypeCn] = useState("");
  const [manualTypeEn, setManualTypeEn] = useState("");
  const [manualQty, setManualQty] = useState(1);
  const [manualMaterialCn, setManualMaterialCn] = useState("");
  const [manualMaterialEn, setManualMaterialEn] = useState("");
  const [manualPrice, setManualPrice] = useState(100);

  // OpenClaw parser live terminal overlay states
