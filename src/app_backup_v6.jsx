...
function App() {
  ...
  const [marketingTab, setMarketingTab] = useState("Overview"); // "Overview", "CaseStudies", "OurStory"
  const [clientPortalTab, setClientPortalTab] = useState("Tracker"); // "Tracker", "Intake"
  const [openFaq, setOpenFaq] = useState(null); // Accordion toggler
  ...
  const triggerIntakeUpload = () => {
    ...
    const demoLogs = [
      ...
      { role: "system", text: "檢測到本地安全網關，讀取上傳的 CAD / PDF 設計圖紙數據...", textEn: "Detected secure local gateway, reading uploaded CAD / PDF drawing stream..." },
      { role: "thought", text: "正在解析圖紙圖層，自動提取家具三維數據、幾何形狀與裝配結構...", textEn: "Parsing drawing layers, auto-extracting 3D geometric dimensions and assembly joints..." },
      { role: "thought", text: "調用 Gemini 1.5 Pro 多模態模型分析附加留言要求：“40把大堂扶手椅、20把單人椅。要求符合英國 Crib 5 消防標準，使用海軍藍亞麻面料搭配黑色啞光金屬椅腿。”", textEn: "Invoked Gemini 1.5 Pro multimodal LLM to parse additional notes: '40 Lobby Armchairs & 20 Club Chairs. Must pass UK Crib 5 fire codes with navy linen and matte black legs.'" },
      { role: "action", text: "調用 `check_swatch_crib5_database` 檢測預選材质 Navy Classic Linen (L-4410) & Royal Blue Velvet (V-9082)。", textEn: "Invoked tool 'check_swatch_crib5_database' to query Navy Classic Linen (L-4410) & Royal Blue Velvet (V-9082)." },
      { role: "observation", text: "核對結果：預選面料物理防火性能優良，完全兼容英國 Crib 5 (Source 5) 法規標準！", textEn: "Verification result: Selected fabrics exhibit superior flame-retardant properties, fully Crib 5 (Source 5) compatible." },
      { role: "action", text: "調用 `create_supabase_order` 創建新項目與訂單草稿。寫入 Supabase 表 `orders` 與 `projects`。", textEn: "Invoked tool 'create_supabase_order' to initialize project draft. Registering in Supabase tables 'orders' and 'projects'..." },
      { role: "observation", text: "Supabase 表同步成功：成功新增項目 `CRAFT-2026-082`，並寫入對應 17 階段初始日誌。", textEn: "Supabase table synchronized successfully: Added project 'CRAFT-2026-082' and initialized 17-stage event log streams." },
      { role: "system", text: "AI CAD 圖紙解析完畢！自動跨越至 S03 階段：物料與技術規格確認中。", textEn: "AI CAD parsing complete! Automatically transition workspace to S03: Tech Specs & BOM verification." }
    ];

    let currentLogIndex = 0;
    const interval = setInterval(() => {
      if (currentLogIndex < demoLogs.length) {
        setParsingLogs(prev => [...prev, demoLogs[currentLogIndex]]);
        currentLogIndex++;
      } else {
        clearInterval(interval);
        setIsIntakeUploading(false);
        setCurrentStageIndex(2); // S03 index
        addLog("OpenClaw Parser", "用戶上傳 CAD 藍圖與留言要求，智能體自動解析並成功創建在途訂單。", "User uploaded CAD blueprint and notes, agent successfully auto-parsed specifications and initialized order.");
      }
    }, 600);
  };
    ...
    if (stageId === "S01" || stageId === "S02" || stageId === "S03" || stageId === "S04") {
      return (
        <div className="glass-card animate-fade-in" style={{ marginBottom: '1rem' }}>
          ...
          <div className="panel-body" style={{ padding: '1rem', display: 'flex', gap: '1.5rem', flexWrap: 'wrap' }}>
            <div className="blueprint-board" style={{ flex: '1 1 500px', minHeight: '310px', position: 'relative' }}>
            ...
            {/* Elegant Side Card: Design Inspiration & Reference */}
            <div className="glass-card" style={{ 
              ...
              <div>
                <div style={{ fontSize: '0.7rem', fontFamily: 'var(--font-tech)', color: 'var(--accent-primary)', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '0.4rem' }}>
                  {lang === "Cn" ? "原始設計手稿輸入" : "Raw Design Intake"}
                </div>
                <div style={{ fontSize: '0.85rem', fontWeight: 500, color: 'var(--text-primary)', marginBottom: '0.8rem' }}>
                  {lang === "Cn" ? "客戶手繪概念參考" : "Client Hand-Drawn Concept"}
                </div>
                <div style={{ width: '100%', height: '140px', borderRadius: '4px', overflow: 'hidden', border: '1px solid var(--glass-border)', position: 'relative' }}>
                  <img src={IMAGES.blueprintIntake} alt="Original Sketch" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                  <div style={{ position: 'absolute', bottom: '6px', right: '6px', background: 'rgba(0,0,0,0.6)', color: '#FAF9F6', padding: '2px 6px', fontSize: '0.6rem', fontFamily: 'monospace' }}>
                    REF_082.JPG
                  </div>
                </div>
              </div>
              <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginTop: '1rem', lineHeight: '1.5', fontWeight: 300 }}>
                {lang === "Cn" 
                  ? "基於客戶上傳的手稿，AI 智能體自動解析骨架、坐姿傾角並匹配實木與冷軋碳鋼框架，完成數字化 CAD 標註建檔。" 
                  : "Based on the client's uploaded sketch, the AI Agent automatically parses skeletal geometry, seat inclinations, and coordinates with steel/hardwood framing to output fully dimensioned CAD specs."}
              </div>
            </div>
            ...
            {stageId === "S04" && !signatureApproved && (
              <button className="btn-premium" style={{ width: '100%', marginTop: '0.8rem', justifyContent: 'center' }} onClick={() => { setSignatureApproved(true); handleChoApproval(); }}>
                ✍️ {lang === "Cn" ? "我已確認規格無误，签字放行" : "Review Specs & Sign-Off Block"}
              </button>
            )}
          </div>
        </div>
      ...
      {currentView === "Marketing" && (
        <div className="animate-fade-in" style={{ paddingBottom: "4rem" }}>
          ...
          <div style={{
            ...
            <span 
              style={{
                ...
                textTransform: "uppercase",
                cursor: "pointer",
                color: marketingTab === "Overview" ? "var(--accent-primary)" : "var(--text-secondary)",
                borderBottom: marketingTab === "Overview" ? "1.5px solid var(--accent-primary)" : "1.5px solid transparent",
                paddingBottom: "0.4rem",
                transition: "all 0.3s cubic-bezier(0.25, 0.46, 0.45, 0.94)"
              }}
              onClick={() => setMarketingTab("Overview")}
            >
              {lang === "Cn" ? "工坊概览 / Overview" : "Overview"}
            </span>
            <span 
              style={{
                fontFamily: "var(--font-tech)",
                ...
                textTransform: "uppercase",
                cursor: "pointer",
                color: marketingTab === "CaseStudies" ? "var(--accent-primary)" : "var(--text-secondary)",
                borderBottom: marketingTab === "CaseStudies" ? "1.5px solid var(--accent-primary)" : "1.5px solid transparent",
                paddingBottom: "0.4rem",
                transition: "all 0.3s cubic-bezier(0.25, 0.46, 0.45, 0.94)"
              }}
              onClick={() => setMarketingTab("CaseStudies")}
            >
              {lang === "Cn" ? "经典案例 / Case Studies" : "Case Studies"}
            </span>
            <span 
              style={{
                fontFamily: "var(--font-tech)",
                ...
                textTransform: "uppercase",
                cursor: "pointer",
                color: marketingTab === "OurStory" ? "var(--accent-primary)" : "var(--text-secondary)",
                borderBottom: marketingTab === "OurStory" ? "1.5px solid var(--accent-primary)" : "1.5px solid transparent",
                paddingBottom: "0.4rem",
                transition: "all 0.3s cubic-bezier(0.25, 0.46, 0.45, 0.94)"
              }}
              onClick={() => setMarketingTab("OurStory")}
            >
              {lang === "Cn" ? "品牌故事 / Our Story" : "Our Story"}
            </span>
          </div>
          ...
          {marketingTab === "Overview" && (
            <div className="animate-fade-in">
              <div className="portal-hero" style={{ 
                ...
                <div>
                  <h1 style={{ textAlign: 'left', fontSize: '2.8rem', marginBottom: '1.5rem', lineHeight: '1.2' }}>
                    {lang === "Cn" ? "高档定制家具，AI 多智能体全自动跟产平台" : "High-End Bespoke Furniture, Driven by Autonomous Multi-Agent Workflows."}
                  </h1>
                  <p style={{ textAlign: 'left', marginBottom: '2rem', fontSize: '1.05rem', lineHeight: '1.7' }}>
                    {lang === "Cn" 
                  ...
                  <div style={{ display: 'flex', gap: '1rem', justifyContent: 'flex-start' }}>
                    <button className="btn-premium" style={{ padding: '0.8rem 1.8rem', fontSize: '0.95rem' }} onClick={() => setCurrentStageView("ClientPortal")}>
                      {lang === "Cn" ? "注册成为会员 ＋ AI选品" : "Join Membership & Design with AI"}
                    </button>
                    <button className="btn-secondary" style={{ padding: '0.8rem 1.8rem', fontSize: '0.95rem' }} onClick={() => setCurrentStageView("Backoffice")}>
                      {lang === "Cn" ? "进入内部 17 阶段跟踪" : "Simulate 17-Stage Tracker"}
                    </button>
                  </div>
                </div>
                
                {/* Right side: Beautiful visual hero photo */}
                <div style={{
                  position: 'relative',
                  borderRadius: '8px',
                  overflow: 'hidden',
                  ...
                  <img 
                    ...
                    alt="Crafton Signature Bespoke Chair" 
                    style={{ 
                      width: '100%', 
                      height: '100%', 
                      objectFit: 'cover'
                  ...
                  <div style={{
                    ...
                    background: 'linear-gradient(to top, rgba(26,25,24,0.75) 0%, rgba(26,25,24,0) 100%)',
                    padding: '1.5rem',
                    color: '#FAF7F2'
                  }}>
                    <span style={{ 
                      ...
                      {lang === "Cn" ? "Crafton 旗艦奢華單椅" : "Crafton Signature Chair"}
                    </span>
                    <span style={{ fontSize: '0.85rem', fontWeight: 300, opacity: 0.7 }}>
                      {lang === "Cn" ? "意式侘寂美學與工藝細節" : "Italian Wabi-Sabi Aesthetics"}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="portal-features-grid">
                <div className="glass-card feature-box" style={{ padding: 0, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
                  <div style={{ height: '180px', width: '100%', overflow: 'hidden', position: 'relative' }}>
                    <img src={IMAGES.wabiTextures} style={{ width: '100%', height: '100%', objectFit: 'cover' }} alt="Crib 5 Gate" />
                    <div style={{ position: 'absolute', top: '1rem', left: '1rem', background: 'rgba(26, 25, 24, 0.75)', color: 'white', padding: '0.4rem 0.8rem', fontSize: '0.75rem', borderRadius: '4px', border: '1px solid rgba(255,255,255,0.1)' }}>
                      🛡️ {lang === "Cn" ? "阻燃合規" : "Fire Compliance"}
                    </div>
                  </div>
                  <div style={{ padding: '2rem' }}>
                    <div className="feature-title" style={{ marginTop: 0 }}>{lang === "Cn" ? "Crib 5 自动消防拦截" : "Crib 5 Anti-Fire Hard Gate"}</div>
                ...
                <div className="glass-card feature-box" style={{ padding: 0, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
                  <div style={{ height: '180px', width: '100%', overflow: 'hidden', position: 'relative' }}>
                    <img src={IMAGES.workflowPhases} style={{ width: '100%', height: '100%', objectFit: 'cover' }} alt="AI CV Inspection" />
                    <div style={{ position: 'absolute', top: '1rem', left: '1rem', background: 'rgba(26, 25, 24, 0.75)', color: 'white', padding: '0.4rem 0.8rem', fontSize: '0.75rem', borderRadius: '4px', border: '1px solid rgba(255,255,255,0.1)' }}>
                      👁️ {lang === "Cn" ? "視覺檢測" : "Computer Vision"}
                    </div>
                  </div>
                  <div style={{ padding: '2rem' }}>
                    <div className="feature-title" style={{ marginTop: 0 }}>{lang === "Cn" ? "AI 视觉质检重合比对" : "AI CV Inspection"}</div>
                    <div className="feature-desc">
                      {lang === "Cn" 
                ...
                <div className="glass-card feature-box" style={{ padding: 0, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
                  <div style={{ height: '180px', width: '100%', overflow: 'hidden', position: 'relative' }}>
                    <img src={IMAGES.blueprintIntake} style={{ width: '100%', height: '100%', objectFit: 'cover' }} alt="OpenClaw Daemon" />
                    <div style={{ position: 'absolute', top: '1rem', left: '1rem', background: 'rgba(26, 25, 24, 0.75)', color: 'white', padding: '0.4rem 0.8rem', fontSize: '0.75rem', borderRadius: '4px', border: '1px solid rgba(255,255,255,0.1)' }}>
                      🤖 {lang === "Cn" ? "智能跟單" : "Autonomous Follow-up"}
                    </div>
                  </div>
                  <div style={{ padding: '2rem' }}>
                    <div className="feature-title" style={{ marginTop: 0 }}>{lang === "Cn" ? "OpenClaw 智能体自动跟单" : "OpenClaw Daemon Follow-up"}</div>
                    <div className="feature-desc">
                      {lang === "Cn" 
            ...
            <div className="animate-fade-in" style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 2rem' }}>
              ...
              <div style={{
                ...
                <img 
                  ...
                  alt="Crafton Luxury Showwall" 
                  style={{ width: '100%', height: '100%', objectFit: 'cover' }}
                />
                <div style={{
                  position: 'absolute',
                  top: 0, left: 0, right: 0, bottom: 0,
                  background: 'linear-gradient(to right, rgba(26,25,24,0.9) 0%, rgba(26,25,24,0.3) 100%)',
                  display: 'flex',
                  flexDirection: 'column',
                  justifyContent: 'center',
                  padding: '3rem'
                  ...
                  <span style={{ 
                    ...
                    {lang === "Cn" ? "全球奢華經典案卷" : "GLOBAL BESPOKE CASE ARCHIVES"}
                  </span>
                  ...
                  <p style={{ color: 'var(--text-muted)', maxWidth: '600px', fontSize: '0.95rem', lineHeight: '1.6', textAlign: 'left' }}>
                    {lang === "Cn" 
                      ? "從倫敦梅費爾的對沖基金頂層沙發，到日內瓦落地窗前的極簡休閒椅。每一個經典項目均通過 Crafton AI 的全鏈路智能追溯，在物理阻燃、幾何精度與材料美學上達到極致融合。" 
                      : "From Geneva's full-bleed glass facades to Mayfair's executive lounges. Every signature case study is fully tracked and optimized by Crafton's AI workflows, guaranteeing structural safety and timeless styling."}
                  </p>
                </div>
              </div>
                  ...
                  >
                    ...
                    <div style={{ padding: '1.5rem' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
                        <h4 style={{ margin: 0, fontSize: '1.15rem', color: 'var(--text-primary)', fontWeight: '500' }}>
          ...
          {marketingTab === "OurStory" && (
            <div className="animate-fade-in" style={{ maxWidth: '1000px', margin: '0 auto', padding: '0 2rem' }}>
              <div style={{
                display: 'grid',
                ...
                <div style={{ textAlign: 'left' }}>
                  <span style={{ 
                    ...
                    {lang === "Cn" ? "我們的品牌故事" : "OUR HERITAGE & VISION"}
                  </span>
                  <h2 style={{ fontSize: '2.4rem', fontWeight: '300', marginBottom: '1.5rem', color: 'var(--text-primary)', lineHeight: '1.2', textAlign: 'left' }}>
                    {lang === "Cn" ? "當侘寂美學遇上人工智能" : "Where Wabi-Sabi Aesthetics Meet Autonomous AI"}
                  </h2>
                  <p style={{ fontSize: '0.95rem', lineHeight: '1.8', color: 'var(--text-secondary)', marginBottom: '1.2rem', textAlign: 'left' }}>
                    {lang === "Cn" 
                      ? "Crafton 起源於倫敦老皮革工坊，深知高級定製家具的靈魂在於「不完美中的極致完美」。我們欣賞天然木皮的微小節疤，感激純麻纖維的粗糙紋理。然而，在全球化供應鏈與各國消防法規的複雜現實下，手藝人往往耗費無數精力在繁瑣的文檔與合規檢測中。" 
                      : "Crafton began in a heritage leather workshop in London, knowing that bespoke furniture lives in the sublime details. We appreciate the unique knots of organic timber and the tactile grain of pure flax. Yet, in a complex global regulatory environment, master craftsmen spend immense time on customs packing and fire code validations."}
                  </p>
                  <p style={{ fontSize: '0.95rem', lineHeight: '1.8', color: 'var(--text-secondary)', textAlign: 'left' }}>
                    {lang === "Cn" 
                      ? "因此，我們創立了 Crafton AI。利用多智能體工作流（Multi-Agent Workflows）與 OpenAI/Gemini 大語言模型，我們將繁重無趣的合規性檢驗、Crib 5阻燃數據庫核對、全自動詢價BOM生成以及每日出廠視覺重合度質檢交給 AI 智能體。讓工匠回歸原木，讓客戶輕鬆掌控進度，實現科技與手工藝最溫柔的融合。" 
                      : "Hence, Crafton AI was born. Leveraging autonomous multi-agent systems and Gemini models, we delegate compliance checks, Crib 5 flame code matches, automatic multi-supplier bidding, and production visual alignment audits to dedicated digital agents. Crafting remains human; compliance becomes autonomous."}
                  </p>
                </div>

                {/* Right side: Magazine photo block */}
                <div style={{ position: 'relative' }}>
                  <div style={{
                    ...
                    <img 
                      ...
                      alt="Heritage Blueprint and Swatches" 
                      style={{ width: '100%', height: '100%', objectFit: 'cover' }}
                    />
                  </div>
                  {/* Subtle caption offset */}
                  <div style={{
                    position: 'absolute',
                    bottom: '-1.5rem',
                    right: '1.5rem',
                    ...
                    <span style={{ fontFamily: 'var(--font-tech)', fontSize: '0.7rem', textTransform: 'uppercase', letterSpacing: '1px', display: 'block', color: 'var(--text-muted)' }}>
                      {lang === "Cn" ? "品牌檔案 04" : "ARCHIVE FOLIO 04"}
                    </span>
                    <span style={{ fontSize: '0.8rem', color: 'var(--text-primary)', fontWeight: '500' }}>
                      {lang === "Cn" ? "設計手稿與面料原樣" : "Original Blueprints & Swatches"}
                    </span>
                  </div>
                </div>
              </div>
        ...
        <div className="animate-fade-in" style={{ padding: "2rem", maxWidth: "1200px", margin: "0 auto" }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
            ...
            <div style={{ background: "rgba(124, 114, 103, 0.08)", padding: '0.5rem 1rem', borderRadius: '2px', border: "1px solid var(--glass-border)", fontSize: '0.85rem' }}>
              {lang === "Cn" ? "当前在途订单 status: " : "Order Tracking: "}
          ...
          {/* Client Portal Sub-Navigation Bar */}
          <div style={{
            display: "flex",
            ...
            <span 
              style={{
                ...
                textTransform: "uppercase",
                cursor: "pointer",
                color: clientPortalTab === "Tracker" ? "var(--accent-cyan)" : "var(--text-secondary)",
                borderBottom: clientPortalTab === "Tracker" ? "2px solid var(--accent-cyan)" : "2px solid transparent",
                paddingBottom: "0.4rem",
                transition: "all 0.3s"
              }}
              onClick={() => setClientPortalTab("Tracker")}
            >
              {lang === "Cn" ? "在途跟單看板 / Tracker" : "Order Tracker"}
            </span>
            <span 
              style={{
                fontFamily: "var(--font-tech)",
                ...
                textTransform: "uppercase",
                cursor: "pointer",
                color: clientPortalTab === "Intake" ? "var(--accent-cyan)" : "var(--text-secondary)",
                borderBottom: clientPortalTab === "Intake" ? "2px solid var(--accent-cyan)" : "2px solid transparent",
                paddingBottom: "0.4rem",
                transition: "all 0.3s"
              }}
              onClick={() => setClientPortalTab("Intake")}
            >
              {lang === "Cn" ? "新單圖紙錄入 / AI CAD Intake" : "CAD & Intake"}
            </span>
          </div>
          ...
          {clientPortalTab === "Intake" && (
            <div className="animate-fade-in" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(350px, 1fr))', gap: '2rem' }}>
              ...
              <div className="glass-card" style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', minHeight: '380px', justifyContent: 'center', padding: '2.5rem' }}>
                <div style={{ textAlign: 'center' }}>
                  <h3 style={{ fontFamily: 'var(--font-tech)', fontSize: '1.1rem', color: 'var(--accent-cyan)', marginBottom: '0.5rem' }}>
                    {lang === "Cn" ? "AI CAD 智能跟產圖紙錄入" : "AI CAD BLUEPRINT INTAKE"}
                  </h3>
                  <p style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', marginBottom: '1.5rem' }}>
                    {lang === "Cn" ? "上傳您的裝配圖紙或留言需求，OpenClaw 將自動對接供應鏈" : "Upload your DWG/PDF drawings and notes, OpenClaw will automatically coordinate production."}
                  </p>
                </div>

                {/* Dotted Dropzone */}
                <div 
                  onClick={!isIntakeUploading ? triggerIntakeUpload : undefined}
                  style={{
                    border: isIntakeUploading ? '2px dashed var(--accent-cyan)' : '2px dashed rgba(124, 114, 103, 0.3)',
                    borderRadius: '6px',
                    padding: '3rem 2rem',
                    textAlign: 'center',
                    cursor: isIntakeUploading ? 'not-allowed' : 'pointer',
                    background: isIntakeUploading ? 'rgba(0, 242, 254, 0.03)' : 'rgba(124, 114, 103, 0.02)',
                    transition: 'all 0.3s cubic-bezier(0.25, 0.46, 0.45, 0.94)',
                    position: 'relative',
                    overflow: 'hidden'
                  }}
                  onMouseEnter={(e) => {
                    if (!isIntakeUploading) {
                      e.currentTarget.style.borderColor = 'var(--accent-cyan)';
                      e.currentTarget.style.background = 'rgba(124, 114, 103, 0.05)';
                    }
                  }}
                  onMouseLeave={(e) => {
                    if (!isIntakeUploading) {
                      e.currentTarget.style.borderColor = 'rgba(124, 114, 103, 0.3)';
                      e.currentTarget.style.background = 'rgba(124, 114, 103, 0.02)';
                    }
                  }}
                >
                  {isIntakeUploading ? (
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1rem' }}>
                      ...
                      <span style={{ fontFamily: 'var(--font-tech)', fontSize: '0.85rem', color: 'var(--accent-cyan)', textTransform: 'uppercase', letterSpacing: '1px' }}>
                        {lang === "Cn" ? "OpenClaw 正在多模態解構中..." : "OpenClaw Deconstructing Blueprint..."}
                      </span>
                      <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>
                        {lang === "Cn" ? "解析幾何圖層、CAD尺寸與面料標準" : "Extracting vector scales, CAD layers, and flammability parameters..."}
                      </span>
                    </div>
                  ) : (
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1rem' }}>
                      <span style={{ fontSize: '2.5rem' }}>📐</span>
                      <span style={{ fontFamily: 'var(--font-tech)', fontSize: '0.85rem', fontWeight: '500', color: 'var(--text-primary)' }}>
                        {lang === "Cn" ? "拖拽 CAD .dwg 文件或 PDF 設計方案至此" : "DRAG & DROP CAD .DWG OR PDF BLUEPRINTS HERE"}
                      </span>
                      <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>
                        {lang === "Cn" ? "或點擊瀏覽本地文件以啟動 AI 解析" : "or click to browse local files to trigger parser"}
                      </span>
                    </div>
                  )}
                </div>

                <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)', textAlign: 'left', lineHeight: '1.4' }}>
                  💡 <strong>{lang === "Cn" ? "跟產須知：" : "Note on Production:"}</strong> {lang === "Cn" ? "系統會依據您上傳的圖紙中標註的面料與椅腳，自動生成對應的 BOM 物料清單，並在背景調用 Crib 5 法規數據庫。若檢測到絲綢等硬性不合規面料，將自動引導在 S05 階段進行替換。" : "The AI parser automatically syncs the drawings to our BOM systems and triggers flammability cross-checks. If non-compliant delicate fabrics are detected, a gateway substitution prompt will automatically trigger at S05."}
                </div>
              </div>

              {/* Right Column: CAD Visual Board & Console */}
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                {/* CAD Viewport Panel */}
                <div className="glass-card" style={{ padding: 0, overflow: 'hidden', position: 'relative', height: '240px' }}>
                  <img 
                    src={IMAGES.blueprintIntake} 
                    style={{ width: '100%', height: '100%', objectFit: 'cover', filter: 'grayscale(0.4) contrast(1.1) brightness(0.9)' }} 
                    alt="CAD Blueprint Viewport" 
                  />
                  <div style={{
                    position: 'absolute',
                    top: 0, left: 0, right: 0, bottom: 0,
                  ...
                  <span style={{
                    ...
                    SCALE 1:25 | DWG LAYERS: 12
                  </span>
                </div>

                {/* Real-time System Console */}
                <div className="glass-card" style={{ flex: 1, padding: '1.2rem', display: 'flex', flexDirection: 'column', background: '#141312', border: '1px solid #2A2826' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #2A2826', paddingBottom: '0.5rem', marginBottom: '0.8rem' }}>
                    <span style={{ fontFamily: 'var(--font-tech)', fontSize: '0.75rem', color: '#8E8B82', letterSpacing: '1px', textTransform: 'uppercase' }}>
                      📟 OpenClaw Parsing Console
                    </span>
                    <span className="stage-badge-dot" style={{ background: isIntakeUploading ? 'var(--accent-cyan)' : '#8E8B82' }}></span>
                  </div>
                  ...
                  >
                    {parsingLogs.length === 0 ? (
                      <span style={{ color: '#5C5954', fontStyle: 'italic' }}>
                        {lang === "Cn" ? "[控制台待機中 - 上傳圖紙以開始解析]" : "[Console Idle - Upload drawing to begin parsing]"}
                      </span>
                    ) : (
                      parsingLogs.map((l, lidx) => {
                        let color = '#8E8B82'; // system
                        let prefix = 'SYS';
                        if (l.role === 'thought') { color = '#A392A4'; prefix = 'THINK'; }
                        if (l.role === 'action') { color = 'var(--accent-cyan)'; prefix = 'ACT'; }
                        if (l.role === 'observation') { color = 'var(--accent-green)'; prefix = 'OBS'; }

                        return (
                          <div key={lidx} style={{ display: 'flex', gap: '0.5rem', alignItems: 'flex-start' }}>
                            <span style={{ color, opacity: 0.8, flexShrink: 0 }}>[{prefix}]</span>
                            <span style={{ color: l.role === 'thought' ? '#B3B0A8' : '#FAF9F6' }}>
                              {lang === "Cn" ? l.text : l.textEn}
                            </span>
                          </div>
                        );
                      })
